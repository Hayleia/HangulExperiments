...
fun typeChar(s: String?, c: Char): String {
	...
	if (lastChar in initials && c in medials) {
		return "${remove(s, 1)}${syllable(initials.indexOf(lastChar), medials.indexOf(c), 0)}"
		...
		if (fin == 0 && c in finals)
			return "${remove(s, 1)}${syllable(ini, med, finals.indexOf(c))}"
		...
		if ((finals[fin] in finalComp) && c in finalComp[finals[fin]]!!.get(0)) {
			...
			return "${remove(s, 1)}${syllable(ini, med, finals.indexOf(tple!!.get(1)[tple[0].indexOf(c)]))}"
		}
		...
		if (fin != 0 && finals[fin] !in finalCompRev && c in medials)
			return "${remove(s, 1)}${syllable(ini, med, 0)}${syllable(initials.indexOf(finals[fin]), medials.indexOf(c), 0)}"

		// if there is a composed final and the new char is a medial, split the old final
		if (finals[fin] in finalCompRev && c in medials)
			return "${remove(s, 1)}${syllable(ini, med, finals.indexOf(finalCompRev.getValue(finals[fin])[0]))}${syllable(initials.indexOf(finalCompRev.getValue(finals[fin])[1]), medials.indexOf(c), 0)}"
		...
		if (medials[med] in medialComp && c in medialComp.getValue(medials[med])[0] && fin == 0) {
			...
			return "${remove(s, 1)}${syllable(ini, medials.indexOf(tple!!.get(1)[tple[0].indexOf(c)]), 0)}"
		}
	}
